...
h = Generator(
    ...
    name = "normal_gamma3"       #name of system
    ).to(device)

gamma = 3 # The performance threshold
c = 1-simple_normal_cdf(gamma).item() # normalization constant
...
h.plot1Dhist(num_samples = 100000, num_bins = 50, factor = c,
    targetLL = simple_normal, gamma = gamma, renorm = True)
plt.show()

# Perform a one sample Kolmogorov-Smirnov test
X = h.sample_X(1000)
F = truncated_normal_cdf(X, gamma)
